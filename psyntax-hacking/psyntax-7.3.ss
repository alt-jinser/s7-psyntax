...
;;;
;;; (gensym id)
;;; returns a unique symbol notated with id each time it's called.
;;; In Chez Scheme, gensym returns a symbol with a "globally" unique name
;;; so that gensyms that end up in the object code of separately compiled
...
(set! sc-expand
  (lambda (x env ctem rtem)
    (let ((env (or env (interaction-environment)))
          (ctem (or ctem '(E)))
          (rtem (or rtem '(E))))
